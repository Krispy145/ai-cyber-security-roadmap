#!/usr/bin/env python3
"""
Update all repository README files with their short_description from manifest.json.

This script:
1. Reads the manifest.json file
2. For each repository, finds the corresponding README.md file
3. Updates the description section while preserving the rest of the README structure
4. Reports which files were updated

Usage:
    python3 scripts/update_readmes.py [--dry-run] [--repo REPO_NAME]
...
def main():
    parser = argparse.ArgumentParser(description='Update repository READMEs from manifest')
    ...
    print(f"{'DRY RUN: ' if args.dry_run else ''}Updating READMEs for {total_count} repositories...")
    print()
